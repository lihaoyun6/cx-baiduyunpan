...
(function(require, define, Promise) {
	...
	unsafeWindow.api = function(data, callback) {
			...
			GM_xmlhttpRequest({
				url: 'http://' + aria2addr + ':' + aria2port + '/' + aria2rpc,
				method: 'POST',
	...
	function showError(msg) {
		...
		var $dialog = $('<div id="errorDialog">' +
						'<h3>cx-baiduyunpan:程序异常</h3>' +
						'<div class="dialog-body"><p>请尝试更新脚本或复制以下信息<a href="https://github.com/lihaoyun6/cx-baiduyunpan/issues" target="_blank">提交issue</a></p>' +
						'<p>Exception: ' + msg + '</p>' +
	...
	define('cx-yunpan:pageInfo', function(require) {
		...
		var PAGE_CONFIG = {
			...
			share: {
				...
				style: function() {
					var styleList = [
						...
						'.cx-yunpan-dropdown-button.g-dropdown-button.button-open .menu{z-index:41;}',
						'.module-share-header .slide-show-header h2{width:230px;}',
						'.KPDwCE .xGLMIab .g-dropdown-button.cx-yunpan-dropdown-button{margin: 0 5px;}'
					];
			...
			enterprise: {
				...
				style: function() {
					var styleList = [
						'.cx-yunpan-dropdown-button .icon-download{background-image: url(/box-static/business-function/infos/icons_z.png?t=1476004014313);}',
						'.cx-yunpan-dropdown-button .g-button:hover .icon-download{background-position: 0px -34px;}'
					];
					GM_addStyle(styleList.join(''));
				}
			}
	...
	define('cx-yunpan:downloadBtnInit', function(require) {
		...
		var menu = [{
			...
			title: 'CX-下载',
			'click': function() {
				var fetchDownLinks = require('cx-yunpan:fetchDownLinks.js');
		...
		$(selector).each(function(i, e) {
			...
			$(e).after(exDlBtn.dom.addClass('cx-yunpan-dropdown-button'));
			exDlBtn.resizeButtonWidth();
		});
		pageInfo.style();
	});

	define('cx-yunpan:fetchDownLinks.js', function (require, exports, module) {
		var $ = require('base:widget/libs/jquerypacket.js');
	...
	define('cx-yunpan:pluginInit.js', function(require) {
		var ctx = require('system-core:context/context.js').instanceForSystem;
		var $ = require('base:widget/libs/jquerypacket.js');
		var pageInfo = require('cx-yunpan:pageInfo');
		var prefix = pageInfo.prefix;
		...
		Promise.all([dmPromise, gjcPromise, ddsPromise]).then(function() {
			try {
				require('cx-yunpan:downloadBtnInit');
				ctx.ui.tip({ mode: 'success', msg: 'cx-baiduyunpan: 插件加载成功' });
			} catch (e) {
				ctx.ui.tip({ mode: 'caution', msg: 'cx-baiduyunpan: 插件加载成功，按钮初始化失败', autoClose: false, hasClose: true });
			}
		}).catch(function(msg) {
	...
	try {
		require('cx-yunpan:pluginInit.js');
